...
function FloatCard({ className, style, label, detail, children }) {
  ...
  const fc = {
    label: { fontFamily: 'var(--font-mono)', fontSize: '0.65rem', textTransform: 'uppercase', color: '#9ca3af', marginBottom: '0.25rem' },
    mono: { fontFamily: 'var(--font-mono)', fontSize: '0.65rem', margin: 0 }
  };
      ...
      >
        ...
        {open && detail && (
          <div style={{ marginTop: '0.6rem', paddingTop: '0.6rem', borderTop: '1px solid var(--color-border)' }}>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'var(--color-green)', margin: 0, lineHeight: 1.6 }}>{detail}</p>
          </div>
        )}
      </button>
    </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem' }}>
              {testimonials.map((t, i) => (
                <div key={i} style={{ padding: '2rem', background: cream, borderRadius: '24px', border: `1px solid ${border}` }}>
                  ...
                  <p style={{ color: muted, fontSize: '0.9rem', margin: 0 }}>{t.role}</p>
                </div>
              ))}
            </div>
          </div>
